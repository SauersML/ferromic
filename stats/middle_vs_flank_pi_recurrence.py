...
logger = logging.getLogger("pi_flanking_analysis_exact_mf_recurrence")
...
# ------------------------------------------------------------------------------
# Core colors for recurrence categories
COLOR_SINGLE_EVENT = "#1f3b78"   # dark blue
COLOR_RECURRENT    = "#8c2d7e"   # reddish purple
COLOR_OVERALL      = "#4c6f8c"   # desaturated teal-blue
...
def run_cluster_rank_test(
    ...
    for seq in sequences:
        ...
        flank = seq.get("flanking_mean")
        middle = seq.get("middle_mean")
...
def perform_statistical_tests(categories: dict, all_sequences_stats: List[dict]) -> dict:
    logger.info("Performing cluster-based rank permutation tests (Middle vs Flanking)...")
...
def _prepare_category_bins(categories: dict) -> Dict[str, List[dict]]:
    """Ensure the bins are present for the recurrence-focused panels."""
    bins = {
...
def _draw_right_key(rax):
    ...
    legend_elements = [
        ...
        {"label": "Single-event loci", "facecolor": COLOR_SINGLE_EVENT, "hatch": OVERLAY_SINGLE_HATCH, "edgecolor": OVERLAY_SINGLE_EDGE, "alpha": ALPHA_VIOLIN, "y": 0.74},
        {"label": "Recurrent loci",    "facecolor": COLOR_RECURRENT,    "hatch": OVERLAY_RECUR_HATCH,  "edgecolor": OVERLAY_RECUR_EDGE,  "alpha": ALPHA_VIOLIN, "y": 0.60},
        # Region Type (Flank/Middle) - using the single-event color to illustrate lightness differences
    ...
    rax.text(X0, 0.88, "Recurrence Type:", ha="left", va="bottom", fontsize=12, color=AX_TEXT, fontweight="bold")
...
def create_mf_quadrant_violins(
    ...
    """Build a two-panel plot contrasting single-event and recurrent loci."""
    logger.info(
        "Creating Middle vs Flank recurrence violins for %s...",
        spec.description,
    )
    start_time = time.time()
    ...
    # Shared y-limits across both panes
    all_vals = []
    for key in bins:
    ...
    # Layout: panels on the top row, legend and colorbar beneath/right
    ...
    color_map = {
        "single_event": COLOR_SINGLE_EVENT,
        "recurrent": COLOR_RECURRENT,
    }
    hatch_map = {
        "single_event": OVERLAY_SINGLE_HATCH,
        "recurrent": OVERLAY_RECUR_HATCH,
    }
    hatch_edge_map = {
        "single_event": OVERLAY_SINGLE_EDGE,
        "recurrent": OVERLAY_RECUR_EDGE,
    }
    ...
    for key in ["single_event", "recurrent"]:
        ...
        hatch_pattern = hatch_map[key]
        hatch_edge_color = hatch_edge_map[key]
        ...
        ax.set_title(f"{title_map[key]} loci", fontsize=14, color=AX_TEXT, pad=18)
        ...
        ax.yaxis.set_major_formatter(
            FuncFormatter(lambda v, pos: format_plain_no_e(v, max_decimals=8))
        )
    ...
    ax_map["single_event"].set_ylabel(
        "Mean Nucleotide Diversity (π)", fontsize=16, color=AX_TEXT
    )
    ...
    cbar.set_label(
        r"$\log_{2}\!\left(\pi_{\mathrm{Middle}} \,/\, \pi_{\mathrm{Flank}}\right)$",
        color=AX_TEXT,
        fontsize=13,
    )
    ...
    new_y = pos.y0 + (pos.height - new_h) * 0.5
    new_x = pos.x0 + pos.width * 0.08
    new_w = pos.width * 0.84
    cax.set_position([new_x, new_y, new_w, new_h])

    out = output_dir / f"pi_mf_recurrence_violins_total_{spec.slug}.pdf"
    try:
    ...
    try:
        ...
        logger.info(f"Saved recurrence figure to {out}")
    except Exception as e:
        logger.error(f"Failed to save recurrence figure: {e}")

    logger.info(f"Recurrence panel plot built in {time.time() - start_time:.2f}s.")
    return fig


def create_overall_violin_plot(
    all_sequences_stats: List[dict],
    test_results: dict,
    output_dir: Path,
    spec: WindowSpec,
) -> Optional[plt.Figure]:
    """Create a single-panel middle vs flank comparison using all loci."""
    logger.info(
        "Creating overall Middle vs Flank violin for %s...",
        spec.description,
    )
    start_time = time.time()

    rows = all_sequences_stats
    if not rows:
        logger.warning("No sequences available for overall plot; skipping.")
        return None

    v_flank = np.asarray([s.get("flanking_mean", np.nan) for s in rows], dtype=float)
    v_mid = np.asarray([s.get("middle_mean", np.nan) for s in rows], dtype=float)

    combined = np.concatenate([v_flank, v_mid])
    ...
    EPS = 1e-12
    log2_vals = []
    for f, m in zip(v_flank, v_mid):
        if np.isfinite(f) and np.isfinite(m):
            log2_vals.append(math.log2((m + EPS) / (f + EPS)))
    log2_vals = np.asarray(log2_vals, dtype=float)
    if log2_vals.size == 0:
        v = 1.0
    else:
        v = float(np.nanpercentile(np.abs(log2_vals), 98))
        if not np.isfinite(v) or v <= 0:
            v = float(np.nanmax(np.abs(log2_vals))) if log2_vals.size else 1.0
            if not np.isfinite(v) or v <= 0:
                v = 1.0
    norm = TwoSlopeNorm(vmin=-v, vcenter=0.0, vmax=v)
    cmap = plt.get_cmap("coolwarm")
    ...
    fig.suptitle(
        f"Middle vs Flank π — Overall ({spec.description})",
        fontsize=16,
        color=AX_TEXT,
        y=0.98,
    )
    gs = fig.add_gridspec(2, 1, height_ratios=[1.0, 0.3], hspace=0.35)
    ax = fig.add_subplot(gs[0, 0])
    cax = fig.add_subplot(gs[1, 0])

    flank_color = adjust_lightness(COLOR_OVERALL, FLANK_LIGHTNESS_FACTOR)
    middle_color = adjust_lightness(COLOR_OVERALL, MIDDLE_LIGHTNESS_FACTOR)
    vals = [v_flank, v_mid]
    pos = [POS_X["Flank"], POS_X["Middle"]]

    ax.set_xlim(-0.55, 1.55)
    ax.set_ylim(y_lo, y_hi)
    ax.set_title("All loci", fontsize=14, color=AX_TEXT, pad=16)

    _draw_two_violins(ax, vals, pos, flank_color, middle_color, hatch_pattern="", hatch_edge_color="none")
    _overlay_boxplots(ax, vals, pos)
    _paired_lines_and_points(
        ax,
        rows,
    ...
    ax.set_ylabel("Mean Nucleotide Diversity (π)", fontsize=16, color=AX_TEXT)

    tr = test_results.get("Overall", {})
    perm_p = _format_p_value_for_annotation(tr.get("mean_p", np.nan))
    label = f"p={perm_p}"
    has_vals = (np.isfinite(v_flank).any() or np.isfinite(v_mid).any())
    finite_combined = combined[np.isfinite(combined)]
    data_max = np.nanmax(finite_combined) if finite_combined.size else y_hi
    y_range = y_hi - y_lo
    y_bracket = min(y_hi - 0.02 * y_range, data_max + 0.06 * y_range)
    _annotate_p_bracket(ax, POS_X["Flank"], POS_X["Middle"], y_bracket, label)
    ...
    pos = cax.get_position(fig)
    new_h = pos.height * 0.60
    new_y = pos.y0 + (pos.height - new_h) * 0.5
    new_x = pos.x0 + pos.width * 0.10
    new_w = pos.width * 0.80
    cax.set_position([new_x, new_y, new_w, new_h])

    out = output_dir / f"pi_mf_overall_violins_total_{spec.slug}.pdf"
    try:
        fig.tight_layout(rect=(0, 0, 1, 0.95))
    except Exception:
        pass
    try:
        plt.savefig(out, dpi=300, bbox_inches="tight", format="pdf")
        logger.info(f"Saved overall figure to {out}")
    except Exception as e:
        logger.error(f"Failed to save overall figure: {e}")

    logger.info(f"Overall plot built in {time.time() - start_time:.2f}s.")
    return fig
...
def main():
    ...
    for spec in WINDOW_SPECS:
        ...
        recurrence_fig = create_mf_quadrant_violins(categories, tests, output_dir, spec)
        overall_fig = create_overall_violin_plot(
            filtered_flanking_stats, tests, output_dir, spec
        )
