...
# --- COVERAGE / RANGE GUARDS (BUG #3) ---
# Imputed inversion dosages are biologically constrained to [0, 2]. A model whose
# predictor matrix is (almost) entirely the missing code (-127) has no real signal:
# missing entries get mean-filled, but with no coverage the column mean is undefined
# and was previously silently filled with 0, so the model predicted from an all-zero
# vector and emitted impossible dosages (e.g. 4.50, -34.99). We now:
#   1. Fail closed (skip + log) any model with zero usable SNP coverage.
#   2. Require a minimum per-SNP and per-sample call rate.
#   3. Clamp predictions into the valid dosage range and record how many were clamped.
DOSAGE_MIN = 0.0
DOSAGE_MAX = 2.0
# A SNP column must have at least this fraction of non-missing genotypes to count as
# "covered". Columns below this contribute no reliable mean and are treated as missing.
MIN_SNP_CALL_RATE = float(os.getenv("MIN_SNP_CALL_RATE", "0.01"))
# A model must have at least this many covered SNP columns to be run at all.
MIN_COVERED_SNPS = int(os.getenv("MIN_COVERED_SNPS", "1"))
# Fraction of a model's SNP columns that must be covered for the model to run.
MIN_COVERED_FRACTION = float(os.getenv("MIN_COVERED_FRACTION", "0.5"))
...
def assess_coverage(X: np.ndarray, min_snp_call_rate: float) -> Tuple[np.ndarray, int, float]:
    """
    Determine which SNP columns have enough non-missing genotypes to be usable.

    Returns:
        covered_mask: bool array (n_snps,) True where call rate >= min_snp_call_rate
        n_covered:    number of covered columns
        overall_call_rate: fraction of non-missing entries across the whole matrix
    """
    n_samples, n_snps = X.shape
    if n_samples == 0 or n_snps == 0:
        return np.zeros(n_snps, dtype=bool), 0, 0.0

    # Count non-missing per column in chunks to stay memmap-friendly.
    non_missing = np.zeros(n_snps, dtype=np.int64)
    chunk = 50000
    for start in range(0, n_samples, chunk):
        end = min(start + chunk, n_samples)
        non_missing += np.sum(X[start:end] != MISSING_VALUE_CODE, axis=0)

    call_rate = non_missing / float(n_samples)
    covered_mask = call_rate >= min_snp_call_rate
    n_covered = int(np.sum(covered_mask))
    overall_call_rate = float(np.sum(non_missing)) / float(n_samples * n_snps)
    return covered_mask, n_covered, overall_call_rate
...
def _process_model_batched(args):
    ...
    try:
        ...
        # --- COVERAGE GUARD (BUG #3): fail closed on (near-)zero coverage ---
        covered_mask, n_covered, overall_call_rate = assess_coverage(X_full, MIN_SNP_CALL_RATE)
        covered_fraction = (n_covered / n_snps) if n_snps > 0 else 0.0
        if n_covered < MIN_COVERED_SNPS or covered_fraction < MIN_COVERED_FRACTION:
            msg = (
                f"insufficient genotype coverage: {n_covered}/{n_snps} SNPs covered "
                f"(fraction={covered_fraction:.2f}, overall call rate={overall_call_rate:.4f}; "
                f"need >= {MIN_COVERED_SNPS} covered and >= {MIN_COVERED_FRACTION:.2f} fraction). "
                f"Predicting from an all-missing matrix yields impossible dosages; skipping."
            )
            print(f"[SKIP] Model {model_name}: {msg}", flush=True)
            return {"model": model_name, "status": "skipped_low_coverage", "error": msg}
        ...
        for i in range(0, n_samples, BATCH_SIZE):
            ...
            preds = clf.predict(X_batch).astype(np.float32).ravel()

            # --- RANGE GUARD (BUG #3): clamp impossible dosages into [0, 2] ---
            out_of_range = (preds < DOSAGE_MIN) | (preds > DOSAGE_MAX)
            n_clamped += int(np.sum(out_of_range))
            np.clip(preds, DOSAGE_MIN, DOSAGE_MAX, out=preds)
        ...
        if n_clamped > 0:
            pct = 100.0 * n_clamped / max(1, len(full_result))
            print(
                f"[WARN] Model {model_name}: clamped {n_clamped} ({pct:.2f}%) predicted "
                f"dosages into [{DOSAGE_MIN}, {DOSAGE_MAX}] (raw model output was out of range).",
                flush=True,
            )
...
def main():
    ...
    with mp.Pool(n_workers) as pool:
        for res in tqdm(pool.imap_unordered(_process_model_batched, pool_args), total=len(todo_models)):
            ...
            elif status == "skipped_low_coverage":
                # Failed closed on coverage: do NOT emit a dosage column for this model.
                skipped_low_coverage.append(res["model"])
            else:
                successful_temp_models.append(res["model"])

    if skipped_low_coverage:
        print(
            f"\n[INFO] Skipped {len(skipped_low_coverage)} model(s) for insufficient "
            f"genotype coverage (no dosages emitted): {', '.join(sorted(skipped_low_coverage))}"
        )
