...
ALPHA = (
    0.50  # elastic-net mixing (0=L2, 1=L1) — keep as-is (no extra sparsity pressure)
)
N_LAM = 30  # reduced lambda path length (was 100)
LAMBDA_MIN_RATIO = 1e-2  # tightened grid tail (was 1e-3)
MAX_ITER = 2000
CLASS_WEIGHT = (
    None  # FIX BIC/weights mismatch: use UNWEIGHTED fit for PGS/BIC consistency
)
NEAR_CONST_SD = 1e-6
...
def _summ(v, name):
    ...
    return (
        f"{name}[n={v.size}] mean={np.nanmean(v):.6g} sd={np.nanstd(v):.6g} "
...
def _read_wide_tsv(path: str) -> pd.DataFrame:
    ...
    if id_col is None:
        raise RuntimeError(
            f"No ID column found in {path}. Expected one of {_ID_CANDIDATES}."
        )
...
def _load_dosages() -> pd.DataFrame:
    ...
    _p(
        f"[load/dosages] Dropped {dropped:,} constant/all-NA inversion columns; kept {kept.shape[1]:,}. "
        f"(elapsed {time.time()-t1:.2f}s)"
    )
    if kept.shape[1] == 0:
        raise RuntimeError("No variable inversion columns after filtering.")
    _p(
        f"[load/dosages] Final shape: {kept.shape[0]:,} x {kept.shape[1]:,} (elapsed {time.time()-t0:.2f}s)"
    )
    return kept
...
def _load_pipeline_covars() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if missing:
        ...
        if not all([cdr_dataset_id, cdr_codename, gcp_project]):
            raise RuntimeError(
                "Covariate caches missing and WORKSPACE_CDR/GOOGLE_PROJECT not set."
            )
    ...
    if REMOVE_RELATED:
        ...
        if not gcp_project:
            raise RuntimeError(
                "GOOGLE_PROJECT must be set to remove related individuals."
            )
        _p("    -> Loading list of related individuals to exclude...")
        related_ids = iox.load_related_to_remove(
        ...
        _p(
            f"[covars] Removed related: {n_before - len(base_df):,} | Remaining: {len(base_df):,}"
        )

    _p(
        f"[covars] Final base covariates shape: {base_df.shape} (elapsed {time.time()-t0:.2f}s)"
    )
...
def _build_phenotype_matrix(sample_index: pd.Index, phenos: list[str]) -> pd.DataFrame:
    _p(
        f"[labels] Building Y for {len(phenos)} phenotypes x {len(sample_index):,} samples"
    )
...
def _align_core(X: pd.DataFrame, base_cov: pd.DataFrame, Y: pd.DataFrame):
    ...
    if len(common) == 0:
        _p(
            "[align DEBUG] |X ∩ Y|        = {:,}".format(
                len(X.index.intersection(Y.index))
            )
        )
        _p(
            "[align DEBUG] |cov ∩ Y|      = {:,}".format(
                len(base_cov.index.intersection(Y.index))
            )
        )
        raise RuntimeError(
            "Empty intersection between dosages, covariates, and labels."
        )
...
def _build_covariates_splits(
    C_base: pd.DataFrame,
    ancestry_df: pd.DataFrame | None,
    train_ids,
    test_ids,
    tag: str,
):
    ...
    dropped = int((~keep).sum())
    if dropped:
        _p(f"[{tag}] [prep/covars] Dropped {dropped} constant covariate cols")
    cov_tr = cov_tr.loc[:, keep]
    cov_te = cov_te.loc[:, keep]
    _p(
        f"[{tag}] [prep/covars] Final: train {cov_tr.shape[1]} cols, test {cov_te.shape[1]} cols"
    )
...
def _lambda_path(
    X: np.ndarray, y: np.ndarray, alpha: float, n: int, lmin_ratio: float
) -> np.ndarray:
...
def _fit_pgs_bic(Ztr: pd.DataFrame, ytr: pd.Series, tag: str):
    ...
    t0 = time.time()
    _p(
        f"[{tag}] [PGS] Lambda sweep (grid={len(lam_grid)}): P={Ztr.shape[1]:,}, N={len(ytr):,}"
    )
    for i, lam in enumerate(lam_grid, 1):
        ...
        if (best is None) or (bic < best["bic"]):
            ...
            inc = 0
            _p(
                f"[{tag}] [PGS] New BEST @step {i}: BIC={best['bic']:.3f} nnz={best['nonzero']} C={best['C']:.4g}"
            )
        else:
            inc += 1
        ...
        if inc >= BIC_EARLY_STOP:
            _p(
                f"[{tag}] [PGS] Early stop: BIC rose {BIC_EARLY_STOP}x | elapsed {time.time()-t0:.1f}s"
            )
            break
    ...
    _p(
        f"[{tag}] [PGS] BEST: step={best['grid_i']}/{len(lam_grid)} lambda={best['lambda']:.4g} "
...
def _pgs_scores(Ztr: pd.DataFrame, Zte: pd.DataFrame, ytr: pd.Series, tag: str):
    ...
    if DEBIAS_REFIT and best["nonzero"] > 0:
        ...
        try:
            ...
            _p(
                f"[{tag}] [PGS] Debiased refit OK. slopes={len(beta_slopes)} "
    ...
    if not np.isfinite(sd) or sd < 1e-12:
        _p(
            f"[{tag}] [PGS/CHK] Train linear predictor ~zero variance (sd={sd:.3e}). No usable PGS."
        )
        return {
            "best": best,
            "pgs_tr": None,
    ...
    try:
        ...
        auc_tr = np.nan
    _p(
        f"[{tag}] [PGS/DIAG] corr(pgs_tr,y_tr)={r if np.isfinite(r) else float('nan'):.4f} | "
...
def _fit_model0_model1_train(
    ...
    _p(
        f"[{tag}] [TRAIN/M1] X1 shape={X1.shape} (added PGS). "
        + _summ(X1["PGS"].values, "PGS_tr")
    )
    m1 = sm.GLM(ytr.values, X1, family=sm.families.Binomial()).fit()
    _p(
        f"[{tag}] [TRAIN/M1] dev={getattr(m1, 'deviance', float('nan')):.6g} "
    ...
    _p(
        f"[{tag}] [TRAIN/TESTS] LRT stat={lrt if np.isfinite(lrt) else float('nan'):.6g} "
...
def _delong_test_paired(
    y_true: np.ndarray, s0: np.ndarray, s1: np.ndarray, alternative="greater"
) -> float:
    a0, v10_0, v01_0 = _fast_delong(y_true, s0)
    a1, v10_1, v01_1 = _fast_delong(y_true, s1)
    if (
        any(v is None for v in (v10_0, v01_0, v10_1, v01_1))
        or np.isnan(a0)
        or np.isnan(a1)
    ):
        return np.nan
...
def _paired_bootstrap_auc(y, s0, s1, B=1000, seed=SEED, tag=""):
    ...
    for b in range(B):
        ...
        if (b + 1) % step_print == 0 or (b + 1) == B:
            ...
            _p(
                f"[{tag}] [TEST/bootstrap] {_bar(b+1, B)} | elapsed {elapsed:.1f}s | ETA {eta:.1f}s"
            )
...
def _evaluate_test(
    m0, m1, cov_te: pd.DataFrame, pgs_te: np.ndarray | None, yte: pd.Series, tag: str
):
    ...
    _p(
        f"[{tag}] [TEST] AUC_M0={auc0:.4f}  AUC_M1={auc1:.4f}  ΔAUC={d_auc:+.4f}  "
...
def _run_one(
    ...
    cfg = {
        ...
        "BIC_EARLY_STOP": BIC_EARLY_STOP,
        "DEBIAS_REFIT": DEBIAS_REFIT,
        "BOOT_B": BOOT_B,
    }
    key = _hash_cfg(cfg)
    ...
    cov_tr, cov_te = _build_covariates_splits(
        C_base, ancestry_df, train_ids, test_ids, ph
    )
    ...
    fit = _fit_model0_model1_train(
        cov_tr, y.loc[train_ids], pgs["pgs_tr"] if pgs["pgs_has_signal"] else None, ph
    )
    ...
    mets_test = _evaluate_test(
        ...
        fit["m1"],
        cov_te,
        pgs["pgs_te"] if pgs["pgs_has_signal"] else None,
        y.loc[test_ids],
        ph,
    )
    ...
    iox.atomic_write_json(
        os.path.join(outdir, "pgs_model.json"),
        {
            "alpha": ALPHA,
            "lambda": pgs["best"]["lambda"],
            "C": pgs["best"]["C"],
            "intercept_penalized": pgs["best"][
                "intercept"
            ],  # for record; not used in PGS
            "nonzero": pgs["best"]["nonzero"],
            "bic": pgs["best"]["bic"],
            "pgs_mu_train": pgs["pgs_mu"],
            "pgs_sd_train": pgs["pgs_sd"],
    ...
    out = {
        "phenotype": ph,
        "TRAIN_LRT_p": fit["p_lrt"],
        "TRAIN_LRT_stat": fit["lrt_stat"],
        "TRAIN_Wald_beta_PGS": fit["beta_pgs"],
        "TRAIN_Wald_se_PGS": fit["se_pgs"],
        "TRAIN_Wald_z": fit["wald_z"],
        "TRAIN_Wald_p_two_sided": fit["p_wald_two"],
        **mets_test,
    ...
    _p(
        f"[{ph}] [done] nnz={pgs['best']['nonzero']} | "
...
def main():
    ...
    _p(
        f"[init] N_WORKERS={N_WORKERS}  ALPHA={ALPHA}  N_LAM={N_LAM}  TEST_SIZE={TEST_SIZE}  BOOT_B={BOOT_B}"
    )
    _p(
        "[init] Threading guards: "
    ...
    if not USE_PIPELINE_COVARS:
        raise RuntimeError(
            "This script expects pipeline covariates (Model 0). Set USE_PIPELINE_COVARS=True."
        )
    ...
    if total == 0:
        ...
        with ThreadPoolExecutor(max_workers=N_WORKERS) as ex:
            fut_to_ph = {
                ex.submit(_run_one, ph, X, Y, base_cov, ancestry_df): ph
                for ph in run_list
            }
            for fut in as_completed(fut_to_ph):
                ...
                try:
                    ...
                    _p(
                        f"[progress] {_bar(done, total)} | elapsed {elapsed:.1f}s | ETA ~{rem:.1f}s"
                    )
