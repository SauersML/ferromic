    ...
    if mode == "lrt_bh":
            ...
            for fn in os.listdir(lrt_dir):
                if fn.endswith(".json") and not fn.endswith(".meta.json"):
                    ...
                    if meta.get("target") != inv:
                        continue
                    rec = pd.read_json(
                        os.path.join(lrt_dir, fn), typ="series"
                    ).to_dict()
                    rows.append(
                        {
                            ...
                            "P_LRT_Overall": pd.to_numeric(
                                rec.get("P_LRT_Overall"), errors="coerce"
                            ),
                        }
                    )
        if rows:
        ...
        if int(mask.sum()) > 0:
            _, q, _, _ = multipletests(
                df.loc[mask, "P_LRT_Overall"], alpha=alpha, method="fdr_bh"
            )
...
def apply_followup_fdr(df, alpha_global, lrt_select_alpha):
    ...
    pcol_overall = (
        "P_LRT_Overall"
        if "P_LRT_Overall" in df.columns
        else ("P_EMP" if "P_EMP" in df.columns else None)
    )
    m_total = (
        int(pd.to_numeric(df[pcol_overall], errors="coerce").notna().sum())
        if pcol_overall
        else 0
    )
    R_selected = int(
        pd.to_numeric(df.get("Sig_Global"), errors="coerce")
        .fillna(False)
        .astype(bool)
        .sum()
    )
    ...
    if R_selected > 0 and alpha_within > 0.0 and "P_LRT_AncestryxDosage" in df.columns:
        ...
        for idx in selected_idx:
            ...
            if (not pd.notna(p_lrt)) or (p_lrt >= lrt_select_alpha):
                continue
            levels_str = (
                str(df.at[idx, "LRT_Ancestry_Levels"])
                if "LRT_Ancestry_Levels" in df.columns
                else ""
            )
            ...
            for anc in map(str.upper, anc_levels):
                ...
                if pcol in df.columns:
                    ...
                    if pd.notna(pval) and reason not in (
                        "insufficient_stratum_counts",
                        "not_selected_by_LRT",
                    ):
            ...
            if pvals:
                _, p_adj_vals, _, _ = multipletests(
                    pvals, alpha=alpha_within, method="fdr_bh"
                )
    ...
    if "Sig_Global" in df.columns:
        ...
        for idx in df.index[df["Sig_Global"] == True].tolist():
            p_lrt = (
                df.at[idx, "P_LRT_AncestryxDosage"]
                if "P_LRT_AncestryxDosage" in df.columns
                else np.nan
            )
            ...
            for anc in anc_levels:
                ...
                if (
                    ...
                    and reason
                    not in ("insufficient_stratum_counts", "not_selected_by_LRT")
                ):
                    sig_groups.append(anc)
            df.at[idx, "FINAL_INTERPRETATION"] = (
                ",".join(sig_groups) if sig_groups else "unable to determine"
            )
