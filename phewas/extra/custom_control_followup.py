...
def _fit_logistic(X: pd.DataFrame, y: pd.Series):
    ...
    for method in ("newton", "bfgs", "cg"):
        try:
            with warnings.catch_warnings():
                warnings.filterwarnings("ignore", category=ConvergenceWarning)
                result = model.fit(method=method, disp=False, maxiter=200)
        except PerfectSeparationError as exc:
            message = _format_logistic_failure(exc, X, y)
            message += "\n    Perfect separation detected; logistic MLE is undefined for this design."
            raise RuntimeError(message) from exc
        except Exception as exc:
            attempts.append((f"logit_{method}", exc))
            warn(f"Logit solver '{method}' raised {exc!r}")
            continue

        if _fit_has_converged(result):
            if method != "newton":
                info(
                    f"Logit MLE using method '{method}' succeeded after alternative solvers were needed."
                )
            return result, f"logit_{method}"

        _report_fit_diagnostics(result, f"Logit {method}")
        attempts.append((f"logit_{method}", RuntimeError("did not converge")))
