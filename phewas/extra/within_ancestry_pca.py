...
Typical use inside the workbench, after staging the microarray PLINK trio locally::
    ...
    python -m phewas.extra.within_ancestry_pca fit \\
        --genotypes ./arrays --sites sites/include_sites.tsv \\
        --cohort cohort_person_ids.txt --group eur --out-dir within_ancestry_pcs
...
def components_for(n_samples: int) -> int:
    """Number of components to fit, scaled to how many participants support them.

    Asking for twenty components from a thousand participants estimates noise. Groups
    below the smallest tier get no ancestry-specific components at all and fall back to
    the global ones in the analysis, which is recorded rather than silently applied.
    """
    if n_samples >= 50_000:
        return 20
    if n_samples >= 5_000:
...
def _run_gnomon_fit(
    ...
    print("[fit] " + " ".join(command), flush=True)
    completed = subprocess.run(command, cwd=workdir, text=True, capture_output=True)
    sys.stdout.write(completed.stdout)
    sys.stderr.write(completed.stderr)
    if completed.returncode != 0:
        if "--keep" in completed.stderr and "unexpected argument" in completed.stderr:
            raise SystemExit(
                "This gnomon build does not accept '--keep' on 'fit'. Ancestry-specific "
                "components require fitting on one group at a time, so the flag is a "
                "prerequisite rather than a convenience. Update gnomon."
            )
...
# Suffixes gnomon strips when deriving the stem it names its outputs after.
_GENOTYPE_SUFFIXES = (
    ".vcf.bgz", ".vcf.gz", ".bcf.bgz", ".bcf.gz",
...
def _output_stem(genotype_path: str) -> str:
    """Reproduce gnomon's output stem so its artifacts can be found afterwards."""
    name = os.path.basename(os.path.normpath(genotype_path))
    lowered = name.lower()
    for suffix in _GENOTYPE_SUFFIXES:
...
def _artifact_paths(genotype_path: str) -> dict[str, str]:
    """gnomon writes beside the genotypes, not into the working directory."""
    base = os.path.dirname(os.path.abspath(os.path.normpath(genotype_path)))
    stem = _output_stem(genotype_path)
    return {name: os.path.join(base, f"{stem}.{name}") for name in _FIT_ARTIFACTS}
...
def _collect_fit_artifacts(genotype_path: str, workdir: str, started_at: float) -> dict[str, str]:
    """Move this group's artifacts out of the shared genotype directory.

    Every fit writes to the same stem beside the genotypes, so fitting a second group
    overwrites the first. Relocating immediately makes each group's model self-contained
    and makes a fit that silently failed to produce fresh output detectable.
    """
    produced = _artifact_paths(genotype_path)
    scores_bin = produced["hwe_scores.bin"]
    if not os.path.exists(scores_bin):
        raise SystemExit(
            f"gnomon fit did not write {scores_bin}. Expected the score matrix beside the "
            "genotype data, which is where gnomon places its outputs."
        )
    if os.path.getmtime(scores_bin) < started_at:
        raise SystemExit(
            f"{scores_bin} predates this run; refusing to read a model left over from another "
            "group. Remove the stale artifacts and re-run."
        )

    collected: dict[str, str] = {}
    for name, path in produced.items():
        if os.path.exists(path) and os.path.getmtime(path) >= started_at:
            destination = os.path.join(workdir, name)
            shutil.move(path, destination)
            collected[name] = destination

    required = ("hwe_scores.bin", "hwe_scores.metadata.json")
    absent = [name for name in required if name not in collected]
    if absent:
        raise SystemExit(
            f"gnomon fit produced no fresh {', '.join(absent)}. The score matrix cannot be "
            "read without its metadata sidecar, which carries the matrix shape."
        )
    return collected
...
def fit_group(args: argparse.Namespace) -> None:
    ...
    workdir = os.path.join(out_dir, f"_gnomon_{group}")
    os.makedirs(workdir, exist_ok=True)

    keep_path = args.keep or os.path.join(workdir, f"keep_{group}.txt")
    ...
    components = args.components or components_for(len(ids))
    if components == 0:
        raise SystemExit(
            f"Group '{group}' has only {len(ids):,} participants, below the smallest tier "
            "that supports ancestry-specific components. Leave this group on the global "
            "components and record the fallback in the methods."
        )
    print(f"[fit] group={group} n={len(ids):,} components={components}")

    started_at = time.time()
    _run_gnomon_fit(
        gnomon=args.gnomon,
        genotypes=os.path.abspath(args.genotypes),
    ...
    collected = _collect_fit_artifacts(os.path.abspath(args.genotypes), workdir, started_at)
    scores = _scores_frame(collected, components)
    missing = sorted(set(ids) - set(scores["person_id"]))
    if missing:
        raise SystemExit(
            f"{len(missing)} participants in the keep list have no scores "
    ...
    summary = collected.get("hwe_summary.tsv", "")
    if summary and os.path.exists(summary):
        # Round-trip through pandas' JSON writer so numpy scalars become plain types;
        # to_dict would hand json.dump numpy.int64 values it cannot serialise.
        sidecar["hwe_summary"] = json.loads(
            pd.read_csv(summary, sep="\t").to_json(orient="records")
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    fit.add_argument("--genotypes", required=True, help="PLINK prefix or directory.")
    ...
    fit.add_argument("--components", type=int, help="Override the size-scaled default.")
    fit.add_argument("--maf", type=float, default=0.01)
    fit.add_argument("--no-ld", action="store_true", help="Disable LD normalisation.")
