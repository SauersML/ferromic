...
FIGURE_TASKS: Sequence[FigureTask] = (
    ...
    FigureTask(
        name="Divergence-corrected dxy/da recurrence comparison",
        script=Path("stats/divergence_da_dxy_by_type.py"),
        outputs=(
            Path("data/divergence_da_dxy_by_type.tsv"),
            Path("data/divergence_da_dxy_by_type_stats.tsv"),
        ),
        dependencies=("output.csv", "inv_properties.tsv"),
        note="Reviewer 1: net (da) and absolute (dxy) divergence by recurrence, robust to the FST/pi inverse relation.",
        group="FST",
    ),
    FigureTask(
        name="Divergence dxy/da breakpoint edge-decay",
        script=Path("stats/divergence_edge_decay.py"),
        outputs=(
            Path("data/divergence_edge_decay_spearman.tsv"),
            Path("data/divergence_middle_vs_flank.tsv"),
            Path("data/divergence_middle_vs_flank_stats.tsv"),
        ),
        dependencies=("inv_properties.tsv", "per_site_fst_output.falsta"),
        note="Reviewer 1: per-site dxy/da edge-vs-middle once corrected for diversity.",
        group="FST",
    ),
    FigureTask(
        name="Recurrence effects under architecture controls",
        script=Path("stats/recurrence_architecture_controls.py"),
        outputs=(
            Path("data/recurrence_controls_summary.tsv"),
            Path("data/recurrence_controls_covariates.tsv"),
        ),
        dependencies=("output.csv", "inv_properties.tsv"),
        optional_dependencies=("data/phy_metadata.tsv",),
        python_dependencies=("statsmodels",),
        note="Reviewer 3.3: recurrence contrasts adjusted + matched for length, AF, SNP & CDS density.",
        group="Diversity",
    ),
    FigureTask(
        name="4-fold degenerate-site diversity",
        script=Path("stats/four_fold_pi.py"),
        outputs=(
            Path("data/four_fold_pi_by_inversion.tsv"),
            Path("data/four_fold_pi_tests.tsv"),
        ),
        dependencies=("output.csv", "inv_properties.tsv", "*.phy"),
        note="Reviewer 1: nucleotide diversity at 4-fold synonymous sites vs whole-locus pi.",
        long_running=True,
        group="CDS",
    ),
    FigureTask(
