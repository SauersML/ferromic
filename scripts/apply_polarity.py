...
def main():
    ...
    ap.add_argument("--polarity-table", metavar="PATH",
                    help="use this polarity table instead of data/inversion_polarity.tsv. "
                         "Use with a DELTA table (flip=1 only where old!=new) to re-polarize "
                         "already-polarized committed data to a new convention.")
    ap.add_argument("--include-regenerated", action="store_true",
                    help="also process specs marked regenerated:True (needed for a delta "
                         "re-polarization of committed values, which are not freshly produced).")
    ...
    global POLARITY
    if args.polarity_table:
        POLARITY = os.path.abspath(args.polarity_table)
    ...
    for spec in SPECS:
        if only is not None and spec["path"] not in only:
            continue
        if spec.get("regenerated") and not args.include_regenerated:
