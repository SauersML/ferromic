...
mod tests {
    ...
    #[test]
    fn test_group1_missing_data_segregating_sites() {
        let (variants, sample_names, sample_filter_unfiltered) = setup_group1_missing_data_test();
    ...
    #[test]
    fn test_group1_missing_data_pi() {
        let (variants, sample_names, sample_filter_unfiltered) = setup_group1_missing_data_test();
        ...
        // Variants considered: 1000 and 3000 (2000 excluded due to missing)
        // For haplotype_group=1:
        // Haplotype assignments based on sample_filter_unfiltered:
        // SAMPLE1 hap1: 0, hap2: 1
        // SAMPLE2 hap1: 0, hap2: 1
        // SAMPLE3 hap1: 0, hap2: 0
        // Haplotypes in group1: [1 (Sample1 hap2), 1 (Sample2 hap2), 0 (Sample3 hap1)]
    ...
    #[test]
    fn test_group1_zero_segregating_sites() {
        // Define all haplotypes as 0, resulting in zero '1's
        let variants = vec![
            create_variant(
                1000,
                vec![
                    Some(vec![0, 0]), // Sample1
                    Some(vec![0, 0]), // Sample2
                    Some(vec![0, 0]), // Sample3
                ],
            ),
            create_variant(
        ...
        // Calculate expected allele frequency based on TSV config:
        // All haplotypes have 0, so expected frequency = 0.0
        let allele_frequency_group1 = calculate_inversion_allele_frequency(&sample_filter);
        ...
        // No segregating sites
        let (segsites, w_theta, pi, _n_hap) = result_group1.unwrap();
        let expected_segsites_group1 = 0;
        println!(
            "Number of segregating sites for Group 1 with zero segsites (expected {}): {}",
            expected_segsites_group1, segsites
        ...
        assert!(
            pi_diff < 1e-6,
            "Nucleotide diversity (pi) for Group 1 with zero segsites is incorrect: expected 0.0, got {:.6}",
            pi
        );
    }
}
