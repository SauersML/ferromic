...
Faithful copy of the original ferromic/svbyeye/bin/make_inv_table.py so the
inversion set and per-locus windows exactly match the panTro6-era pipeline.

Output columns: inv_id chrom start end size_bp inverted_af recurrent disease_genes
                win_start win_end flank
...
with open(inv_props) as fh:
    ...
    for r in reader:
        ...
        disease = clean(r[14]); recur = clean(r[21])
...
with open(out_tsv, "w", newline="") as out:
    ...
    w.writerow(["inv_id","chrom","start","end","size_bp","inverted_af",
                "recurrent","disease_genes","win_start","win_end","flank"])
    w.writerows(rows)
